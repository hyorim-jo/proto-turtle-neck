...
export function useRealtimePostureCoach(
  ...
  useEffect(() => {
    ...
    const warningDurationBeforeInterventionMs =
      interventionMode === "beep"
    ...
    if (readySessionKey !== sessionKey) {
      if (warningDurationTimerRef.current == null) {
        ...
        }, warningDurationBeforeInterventionMs);
      }
      return undefined;
    }
...
async function playBeep() {
  ...
  if (Platform.OS === "android") {
    await Notifications.setNotificationChannelAsync(POSTURE_ALERT_CHANNEL_ID, {
      name: "Posture alerts",
